...
dataset = stepDataset(dir_mesh, lst_dir0, LOADprefix='Load0.0', transform=transform, outputFields='damage', varLOAD=False, step=10)
...
if inference == False:
    ...
    trained_model = train_mesh2damage_2(net, init_epoch, num_epochs, np.Inf, loaders,
                                         optimizer, reconLoss, use_cuda, checkpoint_path, best_model_path, dir_tb, num_field=3)
...
for isample in range(nsamples):
    ...
    pred, loss = inference_mesh2damage_2(model, reconLoss, mesh, load, field, use_cuda)
...
for i in range(10):
    ...
    field = torch.unsqueeze(field, 0)
    t0 = time.time()
    pred, loss = inference_mesh2damage_2(model, reconLoss, mesh, load, field)
