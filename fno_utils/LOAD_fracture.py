...
#from neuralop.utils import RangeNormalizer
...
def vtkFieldReaderDS(vtk_name, fieldName='tomo_Volume'):
    reader = vtk.vtkStructuredPointsReader()
    reader.SetFileName(vtk_name)
    reader.Update()
    data = reader.GetOutput()
...
idx_train = np.random.choice(len(x), n_train)  #random shuffle
    x_train1 = torch.from_numpy(x[idx_train,:,:,:,0]).type(torch.float32).clone()
    x_train = torch.nn.functional.interpolate(x_train1, scale_factor=0.5)
    y_train1 = torch.from_numpy(y[idx_train,:,:,:,0]).type(torch.float32).clone()
    y_train = torch.nn.functional.interpolate(y_train1, scale_factor=0.5) 
'''
...
def load_fracture_mesh_strain( lst_dir0, dir_mesh,
                 n_train, n_tests,
                 batch_size, test_batch_sizes,
                 test_resolutions=[251],
                 train_resolution=251,
...
def load_fracture_mesh_damage( lst_dir0, dir_mesh,
                 n_train, n_tests,
                 batch_size, test_batch_sizes,
                 test_resolutions=[251],
                 train_resolution=251,
...
def load_fracture_mesh_stress_ds( lst_dir0, dir_mesh,
                 n_train, n_tests,
                 batch_size, test_batch_sizes,
                 test_resolutions=[126],
                 train_resolution=126,
                 grid_boundaries=[[0,1],[0,1]],
                 positional_encoding=True,
                 encode_input=True,
                 encode_output=True,
    ...
    n_test = n_tests[0]  #currently, only 1 resolution possible
    idx_test = np.random.choice(np.setdiff1d(np.arange(len(x)),idx_train), n_test)
    x_test1 = torch.from_numpy(x[idx_test,:,:,:,0]).type(torch.float32).clone()
    x_test = torch.nn.functional.interpolate(x_test1, scale_factor=0.5)
    y_test1 = torch.from_numpy(y[idx_test,:,:,:,0]).type(torch.float32).clone()
    y_test = torch.nn.functional.interpolate(y_test1, scale_factor=0.5)
    ## input encoding
    if encode_input:
        if encoding == 'channel-wise':
            reduce_dims = list(range(x_train.ndim))
...
def load_fracture_mesh_strain_ds( lst_dir0, dir_mesh,
                 n_train, n_tests,
                 batch_size, test_batch_sizes,
                 test_resolutions=[251],
                 train_resolution=251,
    ...
    idx_train = np.random.choice(len(x), n_train)  #random shuffle
    x_train1 = torch.from_numpy(x[idx_train,:,:,:,0]).type(torch.float32).clone()
    x_train2 = torch.nn.functional.interpolate(x_train1, scale_factor=0.5)
    x_train = torch.nn.functional.interpolate(x_train2, scale_factor=2)
    y_train1 = torch.from_numpy(y[idx_train,:,:,:,0]).type(torch.float32).clone()
    y_train2 = torch.nn.functional.interpolate(y_train1, scale_factor=0.5) 
    y_train = torch.nn.functional.interpolate(y_train2, scale_factor=2) 
    #
    print(y_train)
    test_batch_size = test_batch_sizes[0]
    test_resolution = test_resolutions[0]
    ...
    idx_test = np.random.choice(np.setdiff1d(np.arange(len(x)),idx_train), n_test)
    x_test1 = torch.from_numpy(x[idx_test,:,:,:,0]).type(torch.float32).clone()
    x_test2 = torch.nn.functional.interpolate(x_test1, scale_factor=0.5)
    x_test = torch.nn.functional.interpolate(x_test2, scale_factor=2)
    y_test1 = torch.from_numpy(y[idx_test,:,:,:,0]).type(torch.float32).clone()
    y_test2 = torch.nn.functional.interpolate(y_test1, scale_factor=0.5)
    y_test = torch.nn.functional.interpolate(y_test2, scale_factor=0.5)
...
def load_fracture_mesh_damage_ds( lst_dir0, dir_mesh,
                 n_train, n_tests,
                 batch_size, test_batch_sizes,
                 test_resolutions=[251],
                 train_resolution=251,
...
#curve_paths = (r"D:\data\L0.05_vf0.3\h0.0002",
...
'''def load_fracture_mesh_damage(lst_dir0, dir_mesh,
    ...
    x = list()
    y = list()
    for mesh_path, curve_path in zip(mesh_paths, curve_paths):

        #loop over folders in mesh directory to set a same order and have them correspond
        for folder in os.listdir(curve_path):
            #if folder == '.DS_Store':
             #   continue
            ...
            # output: stress (filename)
            registerFileName(lst_damage=out_field,
                             fprefix=f'{curve_path}/{folder}/{LOADprefix}',
                             loadstep=loadstep)
